...
class MODEL:
	...
	def main(self, img):
		...
		with tf.variable_scope('dilated_conv', reuse = tf.AUTO_REUSE) :
			...
			pool1 = tf.keras.layers.MaxPool2D(3, padding=p, strides=3)(conv1)
			print("block1", pool1.shape)

			conv2 = conv2d(256,5,p,act,k_init(96,256),1)(pool1)
			pool2 = tf.keras.layers.MaxPool2D(2, padding=p, strides=2)(conv2)
			print("block2", pool2.shape)
	 
			conv3 = conv2d(512,3,p,act,k_init(256,512),1)(pool2)
			conv4 = conv2d(512,3,p,act,k_init(512,512),1)(conv3)
			conv5 = conv2d(512,3,p,act,k_init(512,512),1)(conv4)
			conv6 = conv2d(512,3,p,act,k_init(512,512),1)(conv5)
			conv7 = conv2d(512,3,p,act,k_init(512,512),1)(conv6)
			print("block3", conv7.shape)

			## shape: 27*27 -> 6*6, 3*3, 2*2, 1*1
			# spp36 = tf.keras.layers.MaxPool2D(4, padding="valid")(conv7)
			# spp9 = tf.keras.layers.MaxPool2D(9, padding="valid")(conv7)
			# spp4 = tf.keras.layers.MaxPool2D(13, padding="valid")(conv7)
			# spp1 = tf.keras.layers.MaxPool2D(27, padding="valid")(conv7)
			# print("spp shape", spp36.shape, spp9.shape, spp4.shape, spp1.shape)
			# flat1 = tf.keras.layers.Flatten()(spp36)
			# flat2 = tf.keras.layers.Flatten()(spp9)
			# flat3 = tf.keras.layers.Flatten()(spp4)
			# flat4 = tf.keras.layers.Flatten()(spp1)
			# flat = tf.concat([flat1, flat2, flat3, flat4], axis=-1)
			
			## shape: 27*27 -> 4*4, 2*2, 1*1
			spp16 = tf.keras.layers.MaxPool2D(6, padding="valid")(conv7)
			spp4 = tf.keras.layers.MaxPool2D(13, padding="valid")(conv7)
			spp1 = tf.keras.layers.MaxPool2D(27, padding="valid")(conv7)
			...
			fc2 = tf.keras.layers.Dense(2048, kernel_initializer = k_init(1,1), activation = act)(flat)
