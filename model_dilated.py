...
class MODEL:
    ...
    def main(self, img):
        ...
        with tf.variable_scope('dilated_conv', reuse = tf.AUTO_REUSE) :
            ...
            conv2d = lambda num_f, k_size, pad, act, k_init, strd, dilate: \
            tf.keras.layers.Conv2D(num_f, k_size, padding=pad, activation=act,  kernel_initializer=k_init, strides=strd, dilation_rate=dilate)

            batch_norm = lambda: tf.keras.layers.BatchNormalization()

            # num of filter, filter size, padding, activation, stride, dilation rate
            conv1 = conv2d(6,3,p,act,k_init(3,6),2,1)(img)
            conv2 = conv2d(6,3,p,act,k_init(6,6),1,1)(conv1)
            # pool1 = tf.keras.layers.MaxPool2D(2, padding=p)(conv2)
            # conv2 = batch_norm()(conv2)

            conv3 = conv2d(12,3,p,act,k_init(6,12),1,2)(conv2)
            conv4 = conv2d(24,3,p,act,k_init(12,24),1,4)(conv3)
            conv5 = conv2d(48,3,p,act,k_init(24,48),1,8)(conv4)
            # conv5 = batch_norm()(conv5)

            conv6 = conv2d(96,3,p,act,k_init(48,96),1,16)(conv5)
            conv7 = conv2d(96,3,p,act,k_init(96,96),1,1)(conv6)
            # conv7 = batch_norm()(conv7)

            conv8 = conv2d(3,1,p,act,k_init(96,3),1,1)(conv7)
            
            flat = tf.keras.layers.Flatten()(conv8)
            print("flat shape", flat.shape)

            # fc1 = tf.keras.layers.Dense(4096, kernel_initializer = k_init(1,1), activation = act)(flat)
