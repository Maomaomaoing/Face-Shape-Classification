...
import pandas as pd
from model_incept import MODEL
...
label = [f*np.ones(len(file[f]), dtype = np.int32) for f in range(len(file))]
	...
	for e in range(EPOCH):
		...
		if (e+1) % 5 ==0 or e == 0: 
			...
			for testing, testing_label in zip(all_testing, all_testing_label):
				...
				sess.run(model.dataset_iter_test.initializer, feed_dict={model.img: testing, model.label: testing_label})
				acc_write= []
				for step in range( int( n_test / TEST_BATCH_SIZE ) ):
			...
			if np.mean(train_acc) > 0.95:
				saver.save(sess, './incept_model/'+ 'face_' + str(e+1) + '.ckpt')
			...
			out_df.to_csv("res_incept.csv", index=False)
