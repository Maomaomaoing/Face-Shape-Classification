...
import pandas as pd
from model_dilated import MODEL
...
os.environ['CUDA_VISIBLE_DEVICES']='1' #use which GPU device
	...
	for e in range(EPOCH):
		...
		if (e+1) % 5 ==0 or e == 0: 
			...
			if np.mean(train_acc) > 0.95:
				saver.save(sess, './dilated_model/'+ 'face_' + str(e+1) + '.ckpt')
			...
			out_df.to_csv("res_dilated.csv", index=False)
